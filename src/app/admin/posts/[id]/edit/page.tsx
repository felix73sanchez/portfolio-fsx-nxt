...
export default function EditPostPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        setError(data.error || 'Error al actualizar articulo');
      }
    } catch (_err) {
  ...
  if (loading) {
    return (
      <AdminLayout title="Editar Articulo" subtitle="Cargando...">
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p style={{ color: 'var(--gray)' }}>Cargando articulo...</p>
          </div>
        </div>
      </AdminLayout>
    );
  ...
  if (error && !post) {
    return (
      <AdminLayout title="Editar Articulo">
        <div className="text-center py-16">
          ...
          <h2 className="text-xl font-semibold mb-2" style={{ color: 'var(--fg)' }}>Articulo no encontrado</h2>
          <p className="mb-4" style={{ color: 'var(--gray)' }}>{error}</p>
          <Link href="/admin/posts" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium transition-all"
  ...
  return (
    ...
    >
      <p className="text-sm mb-6" style={{ color: 'var(--gray)' }}>
        Modifica el contenido de tu articulo
      </p>
      ...
      <form onSubmit={handleSubmit} className="admin-form-constrained">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Description */}
            <div>
              <label className="block text-sm font-medium mb-2">Descripcion breve</label>
              <textarea
                value={description}
            ...
            <div>
              ...
              {!preview && (
                <div className="flex flex-wrap gap-1 mb-2 p-2 rounded-t-lg" style={{ background: 'var(--light-gray)', borderBottom: '1px solid var(--border)' }}>
                  ...
                  <button type="button" onClick={() => insertMarkdown('\n## ')} className="p-2 rounded hover:bg-[var(--border)] transition text-sm font-bold" title="Titulo H2">H2</button>
                  <button type="button" onClick={() => insertMarkdown('\n### ')} className="p-2 rounded hover:bg-[var(--border)] transition text-sm font-bold" title="Titulo H3">H3</button>
                  <div className="w-px mx-1" style={{ background: 'var(--border)' }}></div>
                  ...
                  <button type="button" onClick={() => insertMarkdown('`$1`', true)} className="p-2 rounded hover:bg-[var(--border)] transition" title="Codigo inline">
                  ...
                  <button type="button" onClick={() => insertMarkdown('\n```\n$1\n```\n', true)} className="p-2 rounded hover:bg-[var(--border)] transition text-xs font-mono" title="Bloque de codigo">{'{ }'}</button>
              ...
              {preview ? (
                ...
                >
                  {content ? (
                    ...
                    <p style={{ color: 'var(--gray)' }}>El contenido aparecera aqui...</p>
                  )}
                </div>
              ) : (
                <textarea
                  ...
                  required
                  rows={15}
                  className="w-full px-4 py-2.5 rounded-b-lg transition outline-none resize-none font-mono text-sm"
                  style={{
                    background: 'var(--light-gray)',
                    border: '1px solid var(--border)',
                    borderTop: 'none',
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-xl p-5" style={{ background: 'var(--card-bg)', border: '1px solid var(--border)' }}>
              ...
              <label className="flex items-center gap-3 p-3 rounded-lg cursor-pointer transition"
                ...
                <div>
                  ...
                  <p className="text-xs" style={{ color: 'var(--gray)' }}>
                    {published ? 'Visible publicamente' : 'Guardado como borrador'}
                  </p>
                </div>
              </label>
              ...
              <div className="mt-4 pt-4 space-y-3" style={{ borderTop: '1px solid var(--border)' }}>
                <button
                  ...
                  className="w-full py-2.5 font-semibold rounded-lg transition flex items-center justify-center gap-2 disabled:opacity-50 text-sm"
                  style={{ background: 'var(--accent)', color: '#fff' }}
                >
                  {saving ? (
                    <>
                ...
                <Link
                  href={`/blog/${post?.slug}`}
                  target="_blank"
                  className="w-full py-2.5 font-medium rounded-lg transition flex items-center justify-center gap-2 text-sm"
                  style={{ background: 'var(--light-gray)', border: '1px solid var(--border)', color: 'var(--fg)' }}
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  {tags.split(',').filter(t => t.trim()).map((tag, i) => (
                    <span key={i} className="text-xs px-2 py-1 rounded-lg"
                      style={{ background: 'var(--light-gray)', color: 'var(--fg)' }}>#{tag.trim()}</span>
                  ))}
                </div>
              )}
            </div>
              ...
              {!coverImage ? (
                <>
                  <div
                    ...
                    className="cursor-pointer rounded-xl transition-all duration-200 text-center"
                    style={{
                      background: 'var(--light-gray)',
            ...
            {/* Meta Info */}
            {post && (
              <div className="rounded-xl p-5 text-sm" style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', color: 'var(--gray)' }}>
                <p className="mb-2">
