...
export default function ProjectsAdminPage() {
    ...
    if (loading) {
        return (
            <AdminLayout title="Proyectos" subtitle="Cargando...">
    ...
    return (
        ...
        >
            {projects.length === 0 ? (
                <div className="rounded-xl p-16 text-center" style={{ background: 'var(--card-bg)', border: '1px solid var(--border)' }}>
                    ...
                    <h3 className="text-xl font-semibold mb-2" style={{ color: 'var(--fg)' }}>No hay proyectos</h3>
                    <p style={{ color: 'var(--gray)' }} className="mb-4">
                        Comienza agregando tu primer proyecto
                    </p>
                    <Link href="/admin/projects/new" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium transition-all"
                        style={{ background: 'var(--accent)', color: '#fff' }}>
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                        </svg>
                            ...
                            <div className="flex items-start justify-between gap-4">
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-3 mb-2">
                                        <h3 className="text-lg font-semibold truncate" style={{ color: 'var(--fg)' }}>{project.title}</h3>
                                        <span
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {project.technologies.slice(0, 5).map((tech) => (
                                            <span key={tech} className="text-xs px-2 py-1 rounded-lg"
                                                style={{ background: 'var(--light-gray)', color: 'var(--fg)' }}>{tech}</span>
                                        ))}
                                        {project.technologies.length > 5 && (
                                            <span className="text-xs px-2 py-1" style={{ color: 'var(--gray)' }}>
                                                +{project.technologies.length - 5} mas
                                            </span>
                                        )}
                                    </div>
                                </div>
